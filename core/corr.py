...
class CorrBlockFast1D:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4):
        # 에러 나는 CUDA 전용 로직 대신, 안전한 PytorchAlternate 방식을 빌려옵니다.
    ...
    def __call__(self, coords):
        # 원래 여기서 CorrSampler.apply를 호출해서 에러가 났던 겁니다.
        # 그 대신 아래의 안전한 코드가 돌아가게 합니다.
        r = self.radius
        ...
        for i in range(self.num_levels):
            dx = torch.zeros(1).to(coords.device)
            dy = torch.linspace(-r, r, 2*r+1).to(coords.device)
            ...
            # 아래는 이 파일 하단에 이미 정의된 Pytorch용 샘플링 로직입니다.
            # 이 로직은 별도의 CUDA 빌드가 필요 없습니다.
            B, D, H, W = fmap2.shape
            ...
            for grid_slice in grid.unbind(3):
                fmapw_mini = F.grid_sample(fmap2, grid_slice.to(fmap2.dtype), align_corners=True)
            ...
            out_pyramid.append(corr / torch.sqrt(torch.tensor(D).float()))
