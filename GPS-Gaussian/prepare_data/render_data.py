...
def render_data(renderer, data_path, phase, data_id, save_path, cam_nums, res, dis=1.0, is_thuman=False):
    # 1. obj 및 이미지 경로 (THuman2.0_obj 내부)
    obj_path = os.path.join(data_path, phase, data_id, '%s.obj' % data_id)
    img_path = os.path.join(data_path, phase, data_id, 'material0.jpeg')
    ...
    look_at_center = np.array([0, 1.65, 0]) # 카메라 주시점 (얼굴 위치)
    base_cam_pitch = 8 # 카메라 각도 설정
    ...
    if is_thuman:
        # 2. SMPL 경로 수정 (구조에 맞게 THuman2.0_smplx 내부 탐색)
        base_dir = os.path.dirname(data_path)  # '../THuman2.0' 상위 폴더 경로 획득
        smpl_path = os.path.join(base_dir, 'THuman2.0_smplx', phase, data_id, 'smplx_param.pkl')
...
if __name__ == '__main__':
    cam_nums = 16
    scene_radius = 1.2 # 3d모델과 카메라와의 거리(m 단위)
    res = (1024, 1024)
    thuman_root = '../THuman2.0/THuman2.0_obj'
    save_root = '../thuman_120cm_render_data'
