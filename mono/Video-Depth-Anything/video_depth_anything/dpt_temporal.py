...
class DPTHeadTemporal(DPTHead):
    ...
    def forward(self, out_features, patch_h, patch_w, frame_length, micro_batch_size=4, cached_hidden_state_list=None):
        ...
        # 💡 [Fix] GPS-Gaussian 어댑터로 보낼 피처 맵 정의
        # path_3는 시공간 정보가 융합된 고차원 텐서([B, 256, H, W])입니다.
        feature_map = path_3 
        
        # 💡 [Fix] output은 이미 위 if/else 문에서 self.scratch.output_conv2를 통해 계산되었습니다.
        # 존재하지 않는 self.output_conv(h3) 호출을 삭제하여 AttributeError를 해결합니다.

        return output, feature_map, h0 + h1 + h2 + h3