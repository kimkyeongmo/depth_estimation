...
패키지 설치는 GPS-Gaussian 폴더와 Video-Depth-Anything 코드에서 각각 설치해야 함(추후 수정하여 전체적으로 합쳐서 최종 패키지 requirements 제공 예정) \
이때 torch가 겹칠 수 있어서 GPS-Gaussian 먼저 패키지 설치 후 VDA에서 겹치지 않는 패키지만 설치 권장(일단은 AI로 없는 패키지 걸러서 따로 설치하는 것을 권장) \
