...
class GSRegresser(nn.Module):
    def __init__(self, cfg, rgb_dim=3, depth_dim=1, norm_fn='group'):
        ...
        self.out_conv = nn.Conv2d(
            self.decoder_dims[0]+rgb_dim+depth_dim, self.head_dim, kernel_size=3, padding=1
        )
    ...
    def forward(self, img, depth, img_feat):
        ...
        # rot head: eps=1e-6으로 거의 0 벡터 입력 시 폭발 방지
        rot_out = torch.nn.functional.normalize(self.rot_head(out), p=2, dim=1, eps=1e-6)

        # scale head: min=1e-5로 수치 0 방지, max=0.01로 상한 유지
        scale_out = torch.clamp(self.scale_head(out), min=1e-5, max=0.01)
